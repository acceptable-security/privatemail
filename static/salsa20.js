...
function decrypt_salsa_msg(key, nonce, b_msg) {
    var msg = b64ToByteArray(b_msg);
    var sal = new Salsa20(key, nonce);

    var bytes = sal.getBytes(msg.length);
    var out = new Array();

    for(var i = 0; i < msg.length; i++) {
        out[i] = String.fromCharCode(msg[i] ^ bytes[i]);
    }
